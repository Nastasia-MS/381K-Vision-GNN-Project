...
def train_epoch(model, loader, optimizer, criterion, device, coord_diversity_weight=0.001):
    model.train()
    total_loss = 0
    correct = 0
    total = 0
    ...
    for images, labels in tqdm(loader, desc="Training"):
        ...
        # For LearnablePatchHyperViG, model returns (outputs, coords)
        if isinstance(model, LearnablePatchHyperViG):
            outputs, coords = model(images)
            loss = criterion(outputs, labels)
            
            # Add coordinate diversity loss
            coord_diversity_loss = -coords.std()
            loss = loss + coord_diversity_weight * coord_diversity_loss
        else:
